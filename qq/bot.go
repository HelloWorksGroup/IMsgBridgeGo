...
// Init 快速初始化
// 使用 GlobalConfig 初始化账号
// 使用 ./device.json 初始化设备信息
func Init() {
	Device = &client.DeviceInfo{}
	fmt.Println("utils.ReadFile()")
...
func InitWithDeviceJSONContent(deviceJSONContent []byte) {
	var account = GlobalConfig.GetInt64("bot.account")
	var password = GlobalConfig.GetString("bot.password")
...
func GenRandomDevice() {
	...
	err := os.WriteFile("device.json", Device.ToJson(), os.FileMode(0755))
	if err != nil {
...
func Login() error {
	...
	fmt.Println("Login Account: " + strconv.FormatInt(Instance.Uin, 10))
	var loginMethodValue = GlobalConfig.GetString("bot.login-method")
...
func loginResponseProcessor(res *client.LoginResponse) error {
	...
	for {
		...
		case client.SliderNeededError:
			...
			res, err = Instance.QQClient.SubmitTicket(ticket)
			Instance.Disconnect()
			Instance.Release()
...
func fetchCaptcha(id string) string {
	g, err := utils.Request{URL: "https://captcha.go-cqhttp.org/captcha/ticket?id=" + id}.JSON()
	if err != nil {
